...
async def on_message(message):
    ...
    #Displays the prediction trend
    if message.content.startswith('!predict'):
        prediction = predict_price(crypto_code)
        with open(prediction, 'rb') as f:
            picture = discord.File(f)
        await message.channel.send(file=picture)  
        if prediction[-1] > prediction[0]:
            await message.channel.send(
                f"```""Kindly take note that there is a likelihood of an upward adjustment in pricing. We would like to inform you in advance that changes in pricing may occur."+""+"```"
            ) 
        else:
            await message.channel.send(
                f"```""Kindly take note that there is a likelihood of an downward adjustment in pricing. We would like to inform you in advance that changes in pricing may occur."+""+"```"
            )
    ...
    if msg.startswith('!help'):
        await message.channel.send(
            ...
            " !predict - Depicts the overall linear prediction trend of your crypto. ```"
        )
# Returns the prediction value for our cryptocurrency 
def predict_price(code):
    url = f"https://www.alphavantage.co/query?function=DIGITAL_CURRENCY_DAILY&symbol={code}&market=USD&apikey={AV_API_KEY}"
    response = requests.get(url)
    data = json.loads(response.text)
    prices = []
    dates = []
    for date in sorted(data['Time Series (Digital Currency Daily)']):
        prices.append(float(data['Time Series (Digital Currency Daily)'][date]['4a. close (USD)']))
        dates.append(date)
    X = np.arange(len(dates)).reshape(-1, 1)
    y = np.array(prices)
    model = LinearRegression().fit(X, y)
    prediction = model.predict(X)
    trend = "increase" if prediction[-1] > prediction[0] else "decrease"
    fig, ax = plt.subplots()
    ax.plot(dates, prices, label='Actual price')
    ax.plot(dates, prediction, label='Predicted price')
    ax.legend()
    ax.set_xlabel('Date')
    ax.set_ylabel('Price (USD)')
    ax.set_title(f"{code} price prediction")
...
def get_curr_rate():
    url = "https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=" + crypto_code + "&to_currency=USD&apikey=" + "6UY7YZQUG58FZDNR"
...
def get_currency_name(code):
    url = "https://www.alphavantage.co/query?function=CURRENCY_EXCHANGE_RATE&from_currency=" + code + "&to_currency=USD&apikey=" + "6UY7YZQUG58FZDNR"
    response = requests.request("GET", url)
    data = response.json()
...
def get_settings():
    with open('C:\\Users\\demus\\Desktop\\Cryptalks\\settings.json', 'r') as openfile:
